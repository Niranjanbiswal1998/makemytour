...
<%@ page import="travel.SubcategoryDao"%>
<%@ page import="travel.Subcategory"%>
...
Subcategory u=SubcategoryDao.getRecordById(Integer.parseInt(subcat_id));

%>
<div class="text-right" style="width:75%">
 <a class="btn btn-primary font-weight-bold" href="javascript:history.back()">Go Back</a>
	            <a class="btn btn-primary font-weight-bold" href="subcategory_view.jsp">View Category</a>
	        </div>
	        <div style="padding-top:20px"></div>
<form method="post" action="SubCategoryProcessUpdate" enctype="multipart/form-data">
<input class="input-field" type="hidden"  name="subcat_id" id="subcat_id" value="<%= u.getSubcat_id()%>">
<table border="0" width="50%" height="300px" align="center" class="tableshadow">
<tr><td colspan="2" class="toptd">Update Sub Category</td></tr>
		<%
		 Connection con0=new DBConnection().Connect();
		PreparedStatement ps0=con0.prepareStatement("select * from  tbl_category");
        ResultSet rs0=ps0.executeQuery();
		
		%>
		
     	<tr><td class="lefttxt">Category Name</td>
		<td><select id="fk_cat_id"name="fk_cat_id">
        <option value="">Select Category</option>
        <%  while(rs0.next()){ %>
           
            <option value="<%= rs0.getInt(1)%>"><%= rs0.getString(2)%></option>
        <% } %>
        </select></td></tr>
        <% String pic =u.getSubcat_pic(); 
        pic=pic.substring(pic.lastIndexOf("/")-12);
        %>
		<tr><td class="lefttxt">Sub Category Name</td><td><input type="text" name="subcat_name" value="<%= u.getSubcat_name()%>"/></td></tr>
		<tr><td class="lefttxt">Change Pic</td><td><input type="file" name="subcat_pic"><img alt="" src="<%= pic%>" style="width:120px;height:70px"></td>
		<tr><td class="lefttxt">Subcategory Details</td><td><input type="text" name="subcat_details"  value="<%= u.getSubcat_details()%>"/></td></tr>
<tr><td>&nbsp;</td><td ><input type="submit" value="UPDATE" name="sbmt" class="btn btn-primary"/></td></tr>
